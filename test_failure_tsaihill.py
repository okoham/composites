...
#from failurecriteria import PuckActionPlaneMF, layer_strength_enveloppe_2d
...
class Enveloppe2D(unittest.TestCase):
    ...
    def test_s2_s6(self):    
        res_mf = layer_strength_enveloppe_2d(mat, 's2', 's6', criterion)
        fig = plt.figure()
        ax = fig.add_subplot(111)
        values = np.array([item[0] for item in res_mf])
